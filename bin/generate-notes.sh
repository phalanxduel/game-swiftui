...
if [ "${1:-}" = "--help" ] || [ "${1:-}" = "-h" ]; then
  echo "Usage: bin/generate-notes.sh <prev-ref-or-empty> <new-ref> [path-prefix]"
  echo ""
  ...
  echo ""
  echo "Canonical copy: phalanxduel/phalanxduel scripts/release/generate-notes.sh"
  echo "(that repo has no shared script layer with this one, so this is a plain copy —"
  echo "keep them in sync by hand if either changes)."
  exit 0
fi

if [ "$#" -lt 2 ] || [ "$#" -gt 3 ]; then
  echo "Usage: bin/generate-notes.sh <prev-ref-or-empty> <new-ref> [path-prefix]" >&2
  exit 1
fi
